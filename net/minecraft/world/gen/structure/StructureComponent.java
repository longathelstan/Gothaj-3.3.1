...
public abstract class StructureComponent {
   ...
   public static StructureComponent findIntersecting(List<StructureComponent> listIn, StructureBoundingBox boundingboxIn) {
      ...
      do {
         ...
         structurecomponent = (StructureComponent)var3.next();
   ...
   protected boolean isLiquidInStructureBoundingBox(World worldIn, StructureBoundingBox boundingboxIn) {
      ...
      for(j2 = i; j2 <= l; ++j2) {
         for(l2 = k; l2 <= j1; ++l2) {
            if (worldIn.getBlockState(blockpos$mutableblockpos.set(j2, j, l2)).getBlock().getMaterial().isLiquid()) {
               return true;
            }

            if (worldIn.getBlockState(blockpos$mutableblockpos.set(j2, i1, l2)).getBlock().getMaterial().isLiquid()) {
               return true;
            }
         }
      }
      ...
      for(j2 = i; j2 <= l; ++j2) {
         for(l2 = j; l2 <= i1; ++l2) {
            if (worldIn.getBlockState(blockpos$mutableblockpos.set(j2, l2, k)).getBlock().getMaterial().isLiquid()) {
               return true;
            }

            if (worldIn.getBlockState(blockpos$mutableblockpos.set(j2, l2, j1)).getBlock().getMaterial().isLiquid()) {
               return true;
            }
         }
      }
