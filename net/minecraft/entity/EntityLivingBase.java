...
public abstract class EntityLivingBase extends Entity {
         ...
         if (this.getHealth() <= 0.0F) {
            ...
            if (flag) {
               ...
               if (entity != null) {
                  ...
                  this.attackedAtYaw = (float)(MathHelper.atan2(d0, d1) * 180.0D / 3.141592653589793D - (double)this.rotationYaw);
   ...
   private int getArmSwingAnimationEnd() {
      return !Client.INSTANCE.getModuleManager().getModule(AnimationsModule.class).isEnabled() ? (this.isPotionActive(Potion.digSpeed) ? 6 - (1 + this.getActivePotionEffect(Potion.digSpeed).getAmplifier()) * 1 : (this.isPotionActive(Potion.digSlowdown) ? 6 + (1 + this.getActivePotionEffect(Potion.digSlowdown).getAmplifier()) * 2 : 6)) : (int)AnimationsModule.swingSpeed.getValue();
   }
   ...
   public void onLivingUpdate() {
      ...
      if (this.newPosRotationIncrements > 0) {
         offset = this.posX + (this.newPosX - this.posX) / (double)this.newPosRotationIncrements;
