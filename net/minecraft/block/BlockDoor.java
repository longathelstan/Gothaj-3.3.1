...
public class BlockDoor extends Block {
   ...
   public IBlockState getActualState(IBlockState state, IBlockAccess worldIn, BlockPos pos) {
      ...
      if (state.getValue(HALF) == BlockDoor.EnumDoorHalf.LOWER) {
         ...
         if (iblockstate.getBlock() == this) {
            state = state.withProperty(HINGE, (BlockDoor.EnumHingePosition)iblockstate.getValue(HINGE)).withProperty(POWERED, (Boolean)iblockstate.getValue(POWERED));
         }
      } else {
         iblockstate = worldIn.getBlockState(pos.down());
         if (iblockstate.getBlock() == this) {
            state = state.withProperty(FACING, (EnumFacing)iblockstate.getValue(FACING)).withProperty(OPEN, (Boolean)iblockstate.getValue(OPEN));
         }
      }
