...
public abstract class GuiScreen extends Gui implements GuiYesNoCallback {
   ...
   public void drawWorldBackground(int tint) {
      if (this.mc.theWorld != null) {
         drawGradientRect(0.0D, 0.0D, (double)this.width, (double)this.height, -1072689136, -804253680);
   ...
   public void drawBackground(int tint) {
      ...
      while(var4.hasNext()) {
         Particle p = (Particle)var4.next();
         double diffX = p.getX() - (double)Mouse.getEventX();
         double diffY = p.getY() - (double)Mouse.getEventY();
         Math.sqrt(diffX * diffX + diffY * diffY);
         Iterator var12 = this.particles.iterator();

         while(var12.hasNext()) {
            Particle p2 = (Particle)var12.next();
            if (p2 != p) {
