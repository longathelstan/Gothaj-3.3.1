...
public class GuiOverlayDebug extends Gui {
   ...
   protected void renderDebugInfoLeft() {
      ...
      for(int i = 0; i < list.size(); ++i) {
         ...
         if (!Strings.isNullOrEmpty(s)) {
            ...
            this.fontRenderer.drawString(s, 2.0D, (double)i1, 14737632);
         }
      }
   ...
   protected List<String> getDebugInfoRight() {
      ...
      long l = j - k;
      List<String> list = Lists.newArrayList(new String[]{String.format("Java: %s %dbit", System.getProperty("java.version"), this.mc.isJava64bit() ? 64 : 32), String.format("Mem: % 2d%% %03d/%03dMB", l * 100L / i, bytesToMb(l), bytesToMb(i)), String.format("Allocated: % 2d%% %03dMB", j * 100L / i, bytesToMb(j)), "", String.format("CPU: %s", OpenGlHelper.getCpu()), "", String.format("Display: %dx%d (%s)", Display.getWidth(), Display.getHeight(), GL11.glGetString(7936)), GL11.glGetString(7937), GL11.glGetString(7938)});
      ...
      if (this.isReducedDebug()) {
         ...
         if (this.mc.objectMouseOver != null && this.mc.objectMouseOver.typeOfHit == MovingObjectPosition.MovingObjectType.BLOCK && this.mc.objectMouseOver.getBlockPos() != null) {
            ...
            Entry entry;
            String s1;
            for(Iterator var18 = iblockstate.getProperties().entrySet().iterator(); var18.hasNext(); list.add(((IProperty)entry.getKey()).getName() + ": " + s1)) {
