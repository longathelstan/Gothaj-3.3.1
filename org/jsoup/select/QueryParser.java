...
public class QueryParser {
   ...
   private void combinator(char combinator) {
      ...
      Object currentEval;
      if (this.evals.size() == 1) {
         rootEval = currentEval = (Evaluator)this.evals.get(0);
      ...
      switch(combinator) {
      case ' ':
         currentEval = new CombiningEvaluator.And(new Evaluator[]{new StructuralEvaluator.Parent((Evaluator)currentEval), newEval});
         break;
      case '+':
         currentEval = new CombiningEvaluator.And(new Evaluator[]{new StructuralEvaluator.ImmediatePreviousSibling((Evaluator)currentEval), newEval});
         break;
      case ',':
         ...
         break;
      case '>':
         currentEval = new CombiningEvaluator.And(new Evaluator[]{new StructuralEvaluator.ImmediateParent((Evaluator)currentEval), newEval});
         break;
      case '~':
         currentEval = new CombiningEvaluator.And(new Evaluator[]{new StructuralEvaluator.PreviousSibling((Evaluator)currentEval), newEval});
         break;
      default:
         throw new Selector.SelectorParseException("Unknown combinator '%s'", new Object[]{combinator});
      }

      if (replaceRightMost) {
         ((CombiningEvaluator.Or)rootEval).replaceRightMostEvaluator((Evaluator)currentEval);
      } else {
   ...
   private void byTag() {
      ...
      if (tagName.startsWith("*|")) {
         ...
         this.evals.add(new CombiningEvaluator.Or(new Evaluator[]{new Evaluator.Tag(plainTag), new Evaluator.TagEndsWith(tagName.replace("*|", ":"))}));
      } else {
         if (tagName.contains("|")) {
