...
public class MicrosoftAuth {
   ...
   public AccessTokenResponse getAccessToken(String code) throws IOException, MicrosoftException {
      ...
      try {
         response = Jsoup.connect("https://login.live.com/oauth20_token.srf").method(Connection.Method.POST).data("client_id", this.appID).data("client_secret", this.appSecret).data("redirect_uri", this.redirectUri).data("grant_type", "authorization_code").data("code", code).ignoreContentType(true).execute();
   ...
   public AccessTokenResponse refreshAccessToken(String rToken) throws IOException, MicrosoftException {
      ...
      try {
         response = Jsoup.connect("https://login.live.com/oauth20_token.srf").method(Connection.Method.POST).data("client_id", this.appID).data("client_secret", this.appSecret).data("refresh_token", rToken).data("grant_type", "refresh_token").data("redirect_uri", this.redirectUri).ignoreContentType(true).execute();
      } catch (HttpStatusException var6) {
   ...
   public static XboxLiveAuthResponse getXboxLiveAuthResponse(String accessToken, boolean oAuth2) throws IOException, MicrosoftException {
      ...
      try {
         response = Jsoup.connect("https://user.auth.xboxlive.com/user/authenticate").method(Connection.Method.POST).requestBody(requestBody.toString()).header("Content-Type", "application/json").header("Accept", "application/json").ignoreContentType(true).ignoreHttpErrors(true).execute();
      } catch (HttpStatusException var6) {
   ...
   public static XSTSResponse getXSTSResponse(String token) throws IOException, MicrosoftException {
      ...
      requestBody.add("Properties", properties);
      Connection.Response response = null;
      response = Jsoup.connect("https://xsts.auth.xboxlive.com/xsts/authorize").method(Connection.Method.POST).requestBody(requestBody.toString()).header("Content-Type", "application/json").header("Accept", "application/json").ignoreContentType(true).ignoreHttpErrors(true).execute();
      JsonObject jsonResponse = (JsonObject)gson.fromJson(response.body(), JsonObject.class);
      if (jsonResponse.has("XErr")) {
         ...
         default:
            break;
         case 664499193:
            if (var6.equals("2148916235")) {
               var7 = 1;
            }
            break;
         case 664499194:
            if (var6.equals("2148916236")) {
               var7 = 2;
            }
            break;
         case 664499195:
            if (var6.equals("2148916237")) {
               var7 = 3;
            }
            break;
         case 664499196:
            if (var6.equals("2148916238")) {
               var7 = 4;
            }
         }
