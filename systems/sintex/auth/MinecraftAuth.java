...
public class MinecraftAuth {
   ...
   public static String getMinecraftBearerToken(String userHash, String xSTSToken) throws IOException, MinecraftException {
      ...
      try {
         response = Jsoup.connect("https://api.minecraftservices.com/authentication/login_with_xbox").method(Connection.Method.POST).requestBody(requestBody.toString()).header("Content-Type", "application/json").header("Accept", "application/json").ignoreContentType(true).execute();
   ...
   public static MinecraftProfileResponse getMinecraftProfile(String bearerToken) throws IOException, MinecraftException {
      Connection.Response response = Jsoup.connect("https://api.minecraftservices.com/minecraft/profile").method(Connection.Method.GET).header("Authorization", "Bearer " + bearerToken).ignoreContentType(true).ignoreHttpErrors(true).execute();
