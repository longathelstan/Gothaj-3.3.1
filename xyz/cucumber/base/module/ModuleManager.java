...
public class ModuleManager {
   ...
   public Mod getModule(Class modclass) {
      return (Mod)this.modules.stream().filter((m) -> {
         return m.getClass() == modclass;
      }).findFirst().orElse((Object)null);
   }

   public Mod getModule(String name) {
      return (Mod)this.modules.stream().filter((m) -> {
         return m.getName().toLowerCase().replace(" ", "").equals(name.toLowerCase().replace(" ", ""));
      }).findFirst().orElse((Object)null);
   }

   public List<Mod> getModulesByCategory(Category c) {
      return (List)this.modules.stream().filter((m) -> {
