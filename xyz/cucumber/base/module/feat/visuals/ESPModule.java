...
public class ESPModule extends Mod {
   ...
   public void onRenderGui(EventRenderGui e) {
      ...
      for(Iterator var3 = this.entities.entrySet().iterator(); var3.hasNext(); GlStateManager.popMatrix()) {
         Entry<EntityPlayer, PositionUtils> entry = (Entry)var3.next();
         EntityPlayer player = (EntityPlayer)entry.getKey();
         PositionUtils pos = (PositionUtils)entry.getValue();
         GlStateManager.pushMatrix();
         if (this.filled.isEnabled()) {
            RenderUtils.drawRect(pos.getX(), pos.getY(), pos.getX2(), pos.getY2(), ColorUtils.getColor(this.filledColor, (double)(System.nanoTime() / 1000000L), 1.0D, 5.0D));
         }

         this.renderOutline(pos);
         if (this.healthBar.isEnabled()) {
            this.renderHealth(pos.getX() - 2.0D, pos.getY(), pos.getHeight(), -1879048192, 3.0F);
            this.renderHealth(pos.getX() - 2.0D, pos.getY(), pos.getHeight() / (double)this.mc.thePlayer.getMaxHealth() * (double)this.mc.thePlayer.getHealth(), ColorUtils.mix(-65536, -16711936, (double)this.mc.thePlayer.getMaxHealth(), (double)(this.mc.thePlayer.getHealth() < 0.0F ? this.mc.thePlayer.getMaxHealth() : this.mc.thePlayer.getHealth())), 2.5F);
         }
      }
      ...
      if (!this.outline.getMode().toLowerCase().equals("none")) {
         ...
         case -1409151606:
            if (var8.equals("arrows")) {
               double aw = pos.getWidth() / 4.0D;
               this.renderBracket(pos.getX() + aw, pos.getY(), pos.getX(), pos.getY(), pos.getX(), pos.getY() + aw, ColorUtils.getColor(this.outlineColor, (double)(System.nanoTime() / 1000000L), 1.0D, 5.0D), 1.0D);
               this.renderBracket(pos.getX2() - aw, pos.getY(), pos.getX2(), pos.getY(), pos.getX2(), pos.getY() + aw, ColorUtils.getColor(this.outlineColor, (double)(System.nanoTime() / 1000000L), 1.0D, 5.0D), 1.0D);
               this.renderBracket(pos.getX(), pos.getY2() - aw, pos.getX(), pos.getY2(), pos.getX() + aw, pos.getY2(), ColorUtils.getColor(this.outlineColor, (double)(System.nanoTime() / 1000000L), 1.0D, 5.0D), 1.0D);
               this.renderBracket(pos.getX2(), pos.getY2() - aw, pos.getX2(), pos.getY2(), pos.getX2() - aw, pos.getY2(), ColorUtils.getColor(this.outlineColor, (double)(System.nanoTime() / 1000000L), 1.0D, 5.0D), 1.0D);
            }
            break;
         case -35329845:
            if (var8.equals("brackets")) {
               double bw = pos.getWidth() / 4.0D;
               this.renderBracket(new double[]{pos.getX(), pos.getY()}, new double[]{pos.getX(), pos.getY2()}, bw, ColorUtils.getColor(this.outlineColor, (double)(System.nanoTime() / 1000000L), 1.0D, 5.0D), 1.0D);
               this.renderBracket(new double[]{pos.getX2(), pos.getY()}, new double[]{pos.getX2(), pos.getY2()}, -bw, ColorUtils.getColor(this.outlineColor, (double)(System.nanoTime() / 1000000L), 1.0D, 5.0D), 1.0D);
            }
            break;
         case 3154575:
            if (var8.equals("full")) {
               RenderUtils.drawOutlinedRect(pos.getX(), pos.getY(), pos.getX2(), pos.getY2(), ColorUtils.getColor(this.outlineColor, (double)(System.nanoTime() / 1000000L), 1.0D, 5.0D), 1.0F);
            }
         }
      }
