...
public class NoFallModule extends Mod {
         ...
         if (var2.equals("clutch")) {
            if (this.mc.thePlayer.fallDistance > 2.9F) {
               ...
               if (item == -1) {
                  ...
                  this.canWork = true;
                  if (!this.mc.thePlayer.isInWater() && !this.mc.thePlayer.isInWeb && !this.pickup && this.mc.theWorld.getBlockState(new BlockPos(this.mc.thePlayer.posX, this.mc.thePlayer.posY - 2.0D, this.mc.thePlayer.posZ)).getBlock() != Blocks.water && this.mc.theWorld.getBlockState(new BlockPos(this.mc.thePlayer.posX, this.mc.thePlayer.posY - 2.0D, this.mc.thePlayer.posZ)).getBlock() != Blocks.air) {
               ...
               if (this.timer.hasTimeElapsed(150.0D, false)) {
                  RotationUtils.customRots = false;
                  RotationUtils.serverYaw = this.mc.thePlayer.rotationYaw;
                  RotationUtils.serverPitch = this.mc.thePlayer.rotationPitch;
                  this.canWork = false;
   ...
   public void onMotion(EventMotion e) {
      if (e.getType() == EventType.PRE) {
         ...
         case -1357340883:
            ...
            break;
         case -805359837:
            if (var2.equals("vulcan") && (double)this.mc.thePlayer.fallDistance > 2.9D) {
         ...
         case -392544390:
            if (var2.equals("hypixel timer")) {
               if ((double)this.mc.thePlayer.fallDistance - this.mc.thePlayer.motionY > 3.0D) {
         ...
         case 3181391:
            if (var2.equals("grim") && this.mc.thePlayer.fallDistance >= 3.0F) {
               ...
               var10000.motionX *= 0.2D;
               var10000 = this.mc.thePlayer;
               var10000.motionZ *= 0.2D;
               float distance = this.mc.thePlayer.fallDistance;
               if (MovementUtils.isOnGround(2.0D)) {
                  if (distance > 2.0F) {
                     MovementUtils.strafe(0.19F);
                  }

                  if (distance > 3.0F && MovementUtils.getSpeed() < 0.2D) {
                     e.setOnGround(true);
                     distance = 0.0F;
                  }
               }
            ...
            break;
         case 109651721:
            if (var2.equals("spoof") && (double)this.mc.thePlayer.fallDistance > 2.9D) {
               e.setOnGround(true);
               this.mc.thePlayer.fallDistance = 0.0F;
            }
            break;
         case 112097665:
            if (var2.equals("verus") && (double)this.mc.thePlayer.fallDistance > 2.9D) {
               var10000 = this.mc.thePlayer;
   ...
   public void onAirCollide(EventAirCollide e) {
      ...
      switch((var2 = this.mode.getMode().toLowerCase()).hashCode()) {
      case 3181391:
         if (var2.equals("grim") && this.mc.thePlayer.fallDistance >= 3.0F && !this.canWork && this.mc.theWorld.getBlockState(e.getPos()).getBlock() instanceof BlockAir && !this.mc.thePlayer.isSneaking()) {
            double x = (double)e.getPos().getX();
            double y = (double)e.getPos().getY();
            double z = (double)e.getPos().getZ();
            if (y < this.mc.thePlayer.posY) {
               e.setReturnValue(AxisAlignedBB.fromBounds(-15.0D, -1.0D, -15.0D, 15.0D, 1.0D, 15.0D).offset(x, y, z));
            }
         }
      default:
      }
