...
public class FlightModule extends Mod {
   ...
   public NumberSettings speed = new NumberSettings("Speed", 0.5D, 0.1D, 10.0D, 0.01D);
   ...
   public void onDisable() {
      ...
      case -1525012097:
         if (var1.equals("intave boat")) {
            for(int i = 0; i < 3; ++i) {
               this.mc.getNetHandler().getNetworkManager().sendPacketNoEvent(new C03PacketPlayer.C06PacketPlayerPosLook(this.mc.thePlayer.posX - Math.sin(Math.toRadians((double)this.mc.thePlayer.rotationYaw)) * 8.0D, this.mc.thePlayer.posY + 0.1D, this.mc.thePlayer.posZ + Math.cos(Math.toRadians((double)this.mc.thePlayer.rotationYaw)) * 8.0D, this.mc.thePlayer.rotationYaw, this.mc.thePlayer.rotationPitch, true));
            }
         }
         break;
      case 112097665:
      ...
      if (this.outPackets.size() > 0) {
         Iterator var3 = this.outPackets.iterator();
      ...
      if (e.getType() == EventType.PRE) {
         ...
         case -925584629:
            if (var2.equals("vulcan jump")) {
               if (this.flyTicks <= 5) {
                  ...
                  if (this.done) {
                     if (this.flyTicks == 0) {
                        this.mc.thePlayer.setPosition(this.mc.thePlayer.posX, this.mc.thePlayer.posY + 4.0D, this.mc.thePlayer.posZ);
                        ...
                        this.mc.thePlayer.setPosition(this.mc.thePlayer.posX, this.mc.thePlayer.posY + 3.0D, this.mc.thePlayer.posZ);
                     }
                  }
         ...
         case 3181391:
            if (var2.equals("grim")) {
               ...
               if (this.mc.thePlayer.posY <= this.startY) {
                  ...
                  e.setOnGround(true);
                  e.setPitch(90.0F);
                  BlockPos hitPos = new BlockPos(this.mc.thePlayer.posX, this.mc.thePlayer.posY - 1.0D, this.mc.thePlayer.posZ);
                  EnumFacing facing = EnumFacing.DOWN;
                  Vec3 hitVec = this.mc.objectMouseOver.hitVec;
                  float var10000 = (float)(hitVec.xCoord - (double)hitPos.getX());
                  var10000 = (float)(hitVec.yCoord - (double)hitPos.getY());
                  var10000 = (float)(hitVec.zCoord - (double)hitPos.getZ());
         ...
         case 112097665:
            if (var2.equals("verus")) {
               ...
               if (this.done) {
                  ...
                  this.mc.getNetHandler().getNetworkManager().sendPacketNoEvent(new C08PacketPlayerBlockPlacement(new BlockPos(this.mc.thePlayer.posX, this.mc.thePlayer.posY - 1.5D, this.mc.thePlayer.posZ), 1, new ItemStack(Blocks.stone.getItem(this.mc.theWorld, new BlockPos(-1, -1, -1))), 0.0F, 0.94F, 0.0F));
                  MovementUtils.strafe((float)(this.timer.getTime() <= (long)(Client.INSTANCE.getModuleManager().getModule(DisablerModule.class).isEnabled() ? 10000 : 1000) ? this.speed.getValue() : MovementUtils.getBaseMoveSpeed() * 1.2999999523162842D));
         ...
         case 233102203:
            if (var2.equals("vanilla")) {
               this.mc.thePlayer.motionY = this.mc.gameSettings.keyBindJump.pressed ? this.speed.getValue() / 2.0D : (this.mc.gameSettings.keyBindSneak.pressed ? -(this.speed.getValue() / 2.0D) : 0.0D);
         ...
         case 1412601497:
            if (var2.equals("mush mc")) {
               if (this.mc.thePlayer.hurtTime == 0) {
                  this.mc.getNetHandler().getNetworkManager().sendPacketNoEvent(new C03PacketPlayer.C04PacketPlayerPosition(this.mc.thePlayer.posX, this.mc.thePlayer.posY + 3.0001D, this.mc.thePlayer.posZ, false));
   ...
   public void processBlockData() {
      ...
      new BlockPos(this.mc.thePlayer.posX, this.mc.thePlayer.posY - 1.0D, this.mc.thePlayer.posZ);
