      ...
      if (!e.isReset()) {
         if (!Client.INSTANCE.getModuleManager().getModule(ScaffoldModule.class).isEnabled() || !RotationUtils.customRots) {
            ...
            if (this.mc.thePlayer.moveForward != 0.0F) {
               ...
               if (RotationUtils.customRots && ka.MoveFix.getMode().equalsIgnoreCase("Silent") && ka.isEnabled() && ka.allowedToWork && (double)Math.abs(MathHelper.wrapAngleTo180_float((float)Math.toDegrees(MovementUtils.getDirection(this.mc.thePlayer.rotationYaw))) - MathHelper.wrapAngleTo180_float(RotationUtils.serverYaw)) > 67.5D) {
               ...
               if (RotationUtils.customRots && smoothRotation.isEnabled() && !scaffold.isEnabled() && smoothRotation.wasScaffold && (double)Math.abs(MathHelper.wrapAngleTo180_float((float)Math.toDegrees(MovementUtils.getDirection(this.mc.thePlayer.rotationYaw))) - MathHelper.wrapAngleTo180_float(RotationUtils.serverYaw)) > 67.5D) {
                  this.mc.gameSettings.keyBindSprint.pressed = false;
                  this.mc.thePlayer.setSprinting(false);
               }
