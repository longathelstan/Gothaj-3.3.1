...
public class LagRangeModule extends Mod {
   ...
   public NumberSettings pulseDelay = new NumberSettings("pulse Delay", 200.0D, 10.0D, 500.0D, 10.0D);
      ...
      if (!this.mc.isSingleplayer()) {
         try {
            while(this.outPackets.size() > 0) {
               Packet packet = (Packet)this.outPackets.poll();
               this.mc.getNetHandler().getNetworkManager().sendPacketNoEvent(packet);
               if (packet instanceof C03PacketPlayer) {
                  C03PacketPlayer c03 = (C03PacketPlayer)packet;
                  if (this.isTargetCloseOrVisible() && EntityUtils.getDistanceToEntityBoxFromPosition(c03.x, c03.y, c03.z, this.killAura.target) <= 3.0D) {
   ...
   public void fullRelease() {
      if (!this.mc.isSingleplayer()) {
         try {
            while(this.outPackets.size() > 0) {
               this.mc.getNetHandler().getNetworkManager().sendPacketNoEvent((Packet)this.outPackets.poll());
            }
   ...
   public boolean isTargetCloseOrVisible() {
      ...
      if (this.killAura.target == null) {
         ...
         return rayTracedEntity == this.killAura.target || this.mc.objectMouseOver.entityHit == this.killAura.target || EntityUtils.getDistanceToEntityBox(this.killAura.target) <= 3.0D;
      }
   }
