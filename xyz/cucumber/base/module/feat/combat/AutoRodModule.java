   ...
   public void onRotation(EventRotation event) {
      if (!Client.INSTANCE.getModuleManager().getModule(ScaffoldModule.class).isEnabled()) {
         ...
         if (newSlot == -1) {
            ...
            this.target = EntityUtils.getTarget(10.0D, "Players", "Off", 400, Client.INSTANCE.getModuleManager().getModule(TeamsModule.class).isEnabled(), this.ka.TroughWalls.isEnabled(), this.ka.attackInvisible.isEnabled(), this.ka.attackDead.isEnabled());
            if (this.target == null) {
               if (this.cancel) {
                  this.cancel = false;
                  RotationUtils.customRots = false;
            ...
            } else if (this.ka.isEnabled() && !(EntityUtils.getDistanceToEntityBox(this.target) < Math.max(this.ka.Range.value, 4.0D)) && this.target.hurtTime < 3) {
               int delay = 350;
               ...
               if (!this.timer.hasTimeElapsed((double)delay, false)) {
                  ...
                  double multiplier = (double)this.mc.thePlayer.getDistanceToEntity(this.target) / 1.25D;
                  ...
                  double targetPosY = this.target.posY + (double)this.target.getEyeHeight() - 0.4D;
