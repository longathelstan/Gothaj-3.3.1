   ...
   public void onAttack(EventAttack event) {
      if (event.getEntity() != null && event.getEntity() instanceof EntityLivingBase) {
         ...
         if (ent.hurtTime <= 2 && this.mode.getMode().equalsIgnoreCase("Packet")) {
            this.mc.getNetHandler().getNetworkManager().sendPacketNoEvent(new C03PacketPlayer.C04PacketPlayerPosition(this.mc.thePlayer.posX, this.mc.thePlayer.posY + 0.1D, this.mc.thePlayer.posZ, false));
            this.mc.getNetHandler().getNetworkManager().sendPacketNoEvent(new C03PacketPlayer.C04PacketPlayerPosition(this.mc.thePlayer.posX, this.mc.thePlayer.posY + 0.01D, this.mc.thePlayer.posZ, false));
         }
            ...
            case -805359837:
               if (var3.equals("vulcan")) {
                  if (this.attacked) {
                     ...
                     switch(this.ticks) {
                     case 1:
                        event.setY(event.getY() + 0.164D);
                        event.setOnGround(false);
                        break;
                     case 2:
                        event.setY(event.getY() + 0.083D);
                        event.setOnGround(false);
                        break;
                     case 3:
                        event.setY(event.getY() + 0.003D);
                        event.setOnGround(false);
                        this.attacked = false;
                     }
                  } else {
