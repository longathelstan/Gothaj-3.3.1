...
public class AnnoyerModule extends Mod {
   ...
   public NumberSettings delay = new NumberSettings("Delay", 1000.0D, 500.0D, 5000.0D, 250.0D);
   ...
   public void onRotation(EventRotation e) {
      if (!Client.INSTANCE.getModuleManager().getModule(ScaffoldModule.class).isEnabled()) {
         ...
         if (newSlot == -1) {
            ...
            this.target = EntityUtils.getTarget(5.0D, "Players", "Off", 400, Client.INSTANCE.getModuleManager().getModule(TeamsModule.class).isEnabled(), this.ka.TroughWalls.isEnabled(), this.ka.attackInvisible.isEnabled(), this.ka.attackDead.isEnabled());
            if (this.target == null) {
               this.cancel = false;
            } else if (EntityUtils.getDistanceToEntityBox(this.target) <= 3.0D) {
               this.target = null;
               this.cancel = false;
               ...
               float[] rots = RotationUtils.getRotationFromPosition(this.target.posX, this.target.posY - 1.6D, this.target.posZ);
