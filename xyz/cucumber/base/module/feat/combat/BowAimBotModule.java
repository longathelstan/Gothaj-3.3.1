...
public class BowAimBotModule extends Mod {
   ...
   public void onUpdate(EventUpdate e) {
      this.target = EntityUtils.getTarget(80.0D, this.ka.Targets.getMode(), "Off", 400, Client.INSTANCE.getModuleManager().getModule(TeamsModule.class).isEnabled(), this.ka.TroughWalls.isEnabled(), this.ka.attackInvisible.isEnabled(), this.ka.attackDead.isEnabled());
   ...
   public float[] getBowRotations(EntityLivingBase entity) {
      ...
      double d = (double)this.mc.thePlayer.getDistanceToEntity(entity) / 2.5D;
      double posX = this.target.posX + (this.target.posX - this.target.prevPosX) * d - this.mc.thePlayer.posX;
      double posY = this.target.posY + (this.target.posY - this.target.prevPosY) * 1.0D + (double)this.target.height * 0.5D - this.mc.thePlayer.posY - (double)this.mc.thePlayer.getEyeHeight();
      ...
      float neededPitch = (float)(-Math.toDegrees(Math.atan(((double)velocitySq - Math.sqrt((double)velocityPow4 - (double)g * ((double)g * hDistanceSq + 2.0D * posY * (double)velocitySq))) / ((double)g * hDistance))));
      return Float.isNaN(neededPitch) ? RotationUtils.getInstantTargetRotation(entity, entity.posX, entity.posY, entity.posZ, false) : new float[]{yaw, neededPitch};
   }
}
