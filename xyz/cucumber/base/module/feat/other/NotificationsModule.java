...
public class NotificationsModule extends Mod {
   ...
   public void onRenderGui(EventRenderGui e) {
      ...
      while(itr.hasNext()) {
         NotificationsModule.Notification n = (NotificationsModule.Notification)itr.next();
   ...
   public static class Notification {
      ...
      public boolean update(double x, double y, int offset) {
         ...
         if (System.nanoTime() / 1000000L >= (long)this.time) {
            ...
            double w = Fonts.getFont("rb-r").getWidth(this.description) + 40.0D + 5.0D;
            double size = 0.0D;
            if (t > (double)(this.maxTime - 500L)) {
               size = w - Math.pow(w, 1.0D - ((double)this.maxTime - t) / 500.0D);
            } else if (t <= (double)(this.maxTime - 500L) && t >= 500.0D) {
               size = w;
            } else {
               size = Math.pow(w, t / 500.0D);
            }

            this.position.setX(x - size);
            this.animation = (this.animation * 9.0D + (double)(25 * offset)) / 10.0D;
            ...
            RenderUtils.drawRoundedRect(this.position.getX(), this.position.getY(), this.position.getX() + 20.0D, this.position.getY2(), col, 1.0F);
            RenderUtils.drawImage(this.position.getX() + 3.0D, this.position.getY() + 2.5D, 15.0D, 15.0D, icon, -1);
            Fonts.getFont("rb-m").drawString(this.name, this.position.getX() + 23.0D, this.position.getY() + 5.0D, -1);
            Fonts.getFont("rb-r").drawString(this.description + " (" + (double)((int)(t / 100.0D)) / 10.0D + "s)", this.position.getX() + 23.0D, this.position.getY() + 5.0D + 8.0D, -5592406);
            return false;
         }
      }
