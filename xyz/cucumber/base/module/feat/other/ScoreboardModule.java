...
public class ScoreboardModule extends Mod implements Dragable {
   ...
   public ScoreboardModule() {
      this.addSettings(new ModuleSettings[]{this.positionX, this.positionY, this.rounded, this.blur, this.bloom, this.bloomColor, this.backgroundColor, this.outline, this.outlineColor});
   }
   ...
   public void onBlur(EventBlur e) {
      if (this.blur.isEnabled()) {
         ...
         ScoreObjective objective = (ScoreObjective)(scoreobjective != null ? scoreobjective : scoreboard.getObjectiveInDisplaySlot(1));
         if (objective != null) {
      ...
      if (this.bloom.isEnabled()) {
         ...
         if (objective != null) {
            ...
            if (this.bloom.isEnabled()) {
               ...
               int color = ColorUtils.getColor(this.bloomColor, (double)(System.nanoTime() / 1000000L), 0.0D, 5.0D);
               if (this.rounded.isEnabled()) {
   ...
   public void onRenderGui(EventRenderGui e) {
      ...
      if (objective != null) {
         ...
         int height = collection.size() * 9 + 16;
         int color = ColorUtils.getAlphaColor(ColorUtils.getColor(this.backgroundColor, (double)(System.nanoTime() / 1000000L), 0.0D, 5.0D), this.backgroundColor.getAlpha());
         if (this.rounded.isEnabled()) {
            RenderUtils.drawRoundedRect(pos[0], pos[1], pos[0] + width, pos[1] + (double)height, color, 2.0F);
         } else {
            RenderUtils.drawRect(pos[0], pos[1], pos[0] + width, pos[1] + (double)height, color);
         ...
         if (this.outline.isEnabled()) {
            color = ColorUtils.getAlphaColor(ColorUtils.getColor(this.outlineColor, (double)(System.nanoTime() / 1000000L), 0.0D, 5.0D), this.outlineColor.getAlpha());
            if (this.rounded.isEnabled()) {
               RenderUtils.drawOutlinedRoundedRect(pos[0], pos[1], pos[0] + width, pos[1] + (double)height, color, 2.0D, 0.1D);
            } else {
               RenderUtils.drawOutlinedRect(pos[0], pos[1], pos[0] + width, pos[1] + (double)height, color, 1.0F);
            }
         }

         Fonts.getFont("rb-r").drawString(objective.getDisplayName(), pos[0] + width / 2.0D - Fonts.getFont("rb-r").getWidth(objective.getDisplayName()) / 2.0D, pos[1] + 4.0D, -1);
         int j = 0;
         ...
         while(var16.hasNext()) {
            ...
            Fonts.getFont("rb-r").drawString(s1, pos[0] + 3.0D, pos[1] + (double)height - (double)(9 * j), -1);
         }
