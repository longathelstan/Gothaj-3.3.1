...
public class AutoToolModule extends Mod {
   ...
   public void onLegitPlace(EventClick e) {
      if (this.mc.gameSettings.keyBindAttack.isKeyDown()) {
         int var10000 = this.mc.thePlayer.inventory.currentItem;
      }

      if (!this.mc.gameSettings.keyBindAttack.isKeyDown() && this.timer.hasTimeElapsed(60.0D, false) && this.serverSideSlot != this.mc.thePlayer.inventory.currentItem) {
         this.serverSideSlot = this.mc.thePlayer.inventory.currentItem;
         this.reset = false;
         this.tool = -1;
         this.timer.reset();
         ...
         if (this.tool != -1) {
            ...
            if (this.timer.hasTimeElapsed(0.0D, false)) {
