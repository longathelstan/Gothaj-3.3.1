...
public class DisablerModule extends Mod {
   ...
   public void onEnable() {
      ...
      case -741918200:
         if (var1.equals("intave old") && this.autoAura.isEnabled()) {
            ...
            ka.Range.setValue(6.0D);
            ka.interactRange.setValue(6.0D);
         }
         break;
      case 560229671:
   ...
   public void onDisable() {
      ...
      try {
         while(this.packets.size() > 0) {
            this.mc.getNetHandler().getNetworkManager().sendPacketNoEvent((Packet)this.packets.get(0));
            this.packets.remove(this.packets.get(0));
         }

         while(this.packets1.size() > 0) {
            ((Packet)this.packets1.get(0)).processPacket(this.mc.getNetHandler().getNetworkManager().getNetHandler());
            this.packets1.remove(this.packets1.get(0));
         }

         while(this.packets2.size() > 0) {
            ((Packet)this.packets2.get(0)).processPacket(this.mc.getNetHandler().getNetworkManager().getNetHandler());
      ...
      case -741918200:
         if (var1.equals("intave old") && this.autoAura.isEnabled()) {
            KillAuraModule ka = (KillAuraModule)Client.INSTANCE.getModuleManager().getModule(KillAuraModule.class);
            ka.Range.setValue(3.0D);
            ka.interactRange.setValue(4.0D);
         }
         break;
      case 560229671:
         if (var1.equals("verus experimental")) {
   ...
   public void onSendPacket(EventSendPacket e) {
      ...
      case -1840247646:
         if (var2.equals("timer range") && e.getPacket() instanceof C03PacketPlayer) {
            ...
            if (!packet.isMoving() && !packet.getRotating() && !this.mc.thePlayer.isUsingItem() && !Client.INSTANCE.getModuleManager().getModule(ScaffoldModule.class).isEnabled()) {
               e.setCancelled(true);
            }
         }
         break;
      case -1062808493:
         if (var2.equals("mushmc") && e.getPacket() instanceof C03PacketPlayer && this.timer.hasTimeElapsed(1200.0D, true)) {
            packet = (C03PacketPlayer)e.getPacket();
            packet.y -= 9.99999999E8D;
            this.disabled = true;
         }
         break;
      case -863729103:
         ...
         if (var2.equals("intave old")) {
            ...
            if (this.intaveReach.isEnabled()) {
               ...
               if (e.getPacket() instanceof C02PacketUseEntity || e.getPacket() instanceof C0APacketAnimation) {
                  ...
                  entity = EntityUtils.getTargetBox(8.0D, killAura.Targets.getMode(), killAura.switchMode.getMode(), 500, teams.isEnabled(), killAura.TroughWalls.isEnabled(), killAura.attackDead.isEnabled(), killAura.attackInvisible.isEnabled());
                  if (entity == null) {
                     e.setCancelled(true);
                     return;
                  }

                  if (EntityUtils.getDistanceToEntityBox(entity) > 3.0D && killAura.isEnabled()) {
                     e.setCancelled(true);
                  }
               }
               ...
               if (e.getPacket() instanceof C03PacketPlayer && this.move) {
                  ...
                  this.mc.getNetHandler().getNetworkManager().sendPacketNoEvent(new C02PacketUseEntity(entity, C02PacketUseEntity.Action.ATTACK));
               }
            }
         }
         break;
      ...
      case 2065543613:
         if (var2.equals("hypixel test")) {
            if (this.timer.hasTimeElapsed(2000.0D, false) && !this.funPackets.isEmpty()) {
               Packet var3;
               for(Iterator var4 = this.funPackets.iterator(); var4.hasNext(); var3 = (Packet)var4.next()) {
               }

               this.timer.reset();
            }

            if (e.getPacket() instanceof C0FPacketConfirmTransaction || e.getPacket() instanceof C00PacketKeepAlive && !this.timer.hasTimeElapsed(2000.0D, false)) {
         ...
         if (var2.equals("verus experimental")) {
            ...
            if (e.getPacket() instanceof S13PacketDestroyEntities) {
               ...
               if (((S13PacketDestroyEntities)e.getPacket()).getEntityIDs().length != 100) {
                  var5 = (var6 = ((S13PacketDestroyEntities)e.getPacket()).getEntityIDs()).length;
            ...
            if (e.getPacket() instanceof S1BPacketEntityAttach && ((S1BPacketEntityAttach)e.getPacket()).getEntityId() == this.mc.thePlayer.getEntityId() && ((S1BPacketEntityAttach)e.getPacket()).getVehicleEntityId() > 0) {
               this.counter = 0;
            }
         }
      }
   ...
   public void onTick(EventTick e) {
      ...
      switch((var2 = this.mode.getMode().toLowerCase()).hashCode()) {
      case -682800726:
         if (var2.equals("spectate 1") && this.timer.hasTimeElapsed(5000.0D, false)) {
            this.move = false;
            this.disabled = false;
      ...
      case 521375265:
         if (var2.equals("hypixel motion") && this.mc.thePlayer.ticksExisted < 150) {
            this.mc.thePlayer.setPosition(this.mc.thePlayer.posX, this.mc.thePlayer.posY + Math.random() / Math.random() * Math.random() / 3.0D, this.mc.thePlayer.posZ);
         }
         break;
      case 560229671:
         if (var2.equals("verus experimental")) {
            ...
            if (this.mc.thePlayer.ridingEntity != null) {
               Iterator var4 = this.mc.theWorld.getLoadedEntityList().iterator();
               ...
               while(true) {
                  while(true) {
                     ...
                     do {
                        ...
                        } while(!(entity instanceof EntityBoat));

                        deltaX = entity.posX - this.mc.thePlayer.posX;
                        deltaY = entity.posY - this.mc.thePlayer.posY;
                        deltaZ = entity.posZ - this.mc.thePlayer.posZ;
                     } while(Math.sqrt(deltaX * deltaX + deltaY * deltaY + deltaZ * deltaZ) >= 5.0D);
                     ...
                     if (entity == this.mc.thePlayer.ridingEntity) {
                        ...
                        if (this.mc.thePlayer.getCurrentEquippedItem() == null || (double)InventoryUtils.getItemDamage(this.mc.thePlayer.getCurrentEquippedItem()) < 4.0D) {
                           Client.INSTANCE.getCommandManager().sendChatMessage("You need to hold better weapon");
                           return;
                        }
   ...
   public void onHit(EventHit e) {
      ...
      case -741918200:
         if (var2.equals("intave old") && this.intaveReach.isEnabled()) {
            ...
            EntityLivingBase entity = EntityUtils.getTargetBox(8.0D, killAura.Targets.getMode(), killAura.switchMode.getMode(), 500, teams.isEnabled(), killAura.TroughWalls.isEnabled(), killAura.attackDead.isEnabled(), killAura.attackInvisible.isEnabled());
            if (entity == null) {
               return;
            }

            if (EntityUtils.getDistanceToEntityBox(entity) > 3.0D && killAura.isEnabled() && entity.hurtTime <= 1 && this.mc.thePlayer.onGround) {
               this.move = true;
            }
         }
         break;
