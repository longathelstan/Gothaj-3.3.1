...
public class StealerModule extends Mod {
   ...
   private final NumberSettings startDelay = new NumberSettings("Start Delay", 50.0D, 0.0D, 1000.0D, 25.0D);
   private final NumberSettings minDelay = new NumberSettings("Min Delay", 5.0D, 0.0D, 1000.0D, 25.0D);
   private final NumberSettings maxDelay = new NumberSettings("Max Delay", 5.0D, 0.0D, 1000.0D, 25.0D);
   ...
   public StealerModule() {
      this.addSettings(new ModuleSettings[]{this.startDelay, this.minDelay, this.maxDelay, this.stealTrashItems, this.autoClose, this.chestName, this.disableOnWorldChange});
   }
      ...
      if (this.mc.thePlayer.ticksExisted > 60) {
         ...
         if (this.mc.currentScreen instanceof GuiChest) {
            ...
            if (this.decidedTimer == 0) {
               ...
               i = (int)Math.ceil(Math.max(this.minDelay.value, this.maxDelay.value));
               this.decidedTimer = RandomUtils.nextInt(delayFirst, i);
            }
