...
public class PingSpoofModule extends Mod {
   ...
   public NumberSettings delay = new NumberSettings("Delay", 90.0D, 0.0D, 1000.0D, 10.0D);
   ...
   public void onGameLoop(EventGameLoop e) {
      if (!this.mc.isSingleplayer()) {
         Iterator iterator = this.incoming.entrySet().iterator();

         Entry entry;
         while(iterator.hasNext()) {
            entry = (Entry)iterator.next();
            if ((Long)entry.getValue() < System.currentTimeMillis()) {
               ((Packet)entry.getKey()).processPacket(this.mc.getNetHandler().getNetworkManager().getNetHandler());
               iterator.remove();
            }
         }
         ...
         while(iterator.hasNext()) {
            entry = (Entry)iterator.next();
            if ((Long)entry.getValue() < System.currentTimeMillis()) {
               this.mc.getNetHandler().getNetworkManager().sendPacketNoEvent((Packet)entry.getKey());
               iterator.remove();
            }
         }
