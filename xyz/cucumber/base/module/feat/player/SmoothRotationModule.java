...
public class SmoothRotationModule extends Mod {
   ...
   public NumberSettings turnSpeed = new NumberSettings("Turn Speed", 3.0D, 15.0D, 60.0D, 1.0D);
   ...
   public void onRotation(EventTick e) {
      ...
      float random = (float)(this.turnSpeed.getValue() * ThreadLocalRandom.current().nextDouble(0.8D, 1.2D));
      int toStop = 1;
      float lastYaw;
      float lastPitch;
      float smoothness;
      ...
      if (this.wasKillAura && (!this.killAura.isEnabled() || this.killAura.isEnabled() && !this.killAura.allowedToWork)) {
         if (!(RotationUtils.getRotationDifference(RotationUtils.serverYaw, this.mc.thePlayer.rotationYaw) > (float)toStop) && !(RotationUtils.getRotationDifference(RotationUtils.serverPitch, this.mc.thePlayer.rotationPitch) > (float)toStop)) {
            ...
            RotationUtils.serverYaw = (float)((double)RotationUtils.serverYaw + (Math.random() * 2.0D - Math.random() * 2.0D));
            RotationUtils.serverPitch = (float)((double)RotationUtils.serverPitch + (Math.random() * 2.0D - Math.random() * 2.0D));
