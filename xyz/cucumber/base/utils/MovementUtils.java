...
public class MovementUtils {
   ...
   public static boolean isGoingDiagonally() {
      return Math.abs(mc.thePlayer.motionX) > 0.04D && Math.abs(mc.thePlayer.motionZ) > 0.04D;
   }

   public static boolean isLookingDiagonally() {
      return MathHelper.wrapAngleTo180_float(mc.thePlayer.rotationYaw) >= 160.0F || MathHelper.wrapAngleTo180_float(mc.thePlayer.rotationYaw) <= 20.0F || MathHelper.wrapAngleTo180_float(mc.thePlayer.rotationYaw) >= 70.0F && MathHelper.wrapAngleTo180_float(mc.thePlayer.rotationYaw) <= 110.0F;
   }
   ...
   public static double jumpBoostMotion(double motionY) {
      return mc.thePlayer.isPotionActive(Potion.jump) ? motionY + (double)((float)(mc.thePlayer.getActivePotionEffect(Potion.jump).getAmplifier() + 1) * 0.1F) : motionY;
   }
   ...
   public static double getPredictedMotionY(double motionY) {
      return (motionY - 0.08D) * 0.9800000190734863D;
   }
   ...
   public static double roundToOnGround(double posY) {
      return posY - posY % 0.015625D;
   }

   public static double distanceToGround(Vec3 vec3) {
      double playerY = vec3.yCoord;
      return playerY - (double)getBlockBellow(vec3).getY() - 1.0D;
   }
   ...
   public static BlockPos getBlockBellow(Vec3 playerPos) {
      while(playerPos.yCoord > 0.0D) {
         BlockPos blockPos = new BlockPos(playerPos);
         if (!isAir(blockPos)) {
            return blockPos;
         }

         playerPos = playerPos.addVector(0.0D, -1.0D, 0.0D);
      }
   ...
   public static double[] getPredictedPos(float forward, float strafe, double motionX, double motionY, double motionZ, double posX, double posY, double posZ, boolean isJumping) {
      ...
      if (isJumping && mc.thePlayer.onGround && mc.thePlayer.jumpTicks == 0) {
         ...
         if (isSprinting) {
            f6 = mc.thePlayer.rotationYaw * 0.017453292F;
            motionX -= (double)(MathHelper.sin(f6) * 0.2F);
            motionZ += (double)(MathHelper.cos(f6) * 0.2F);
         }
      }
      ...
      if (f7 >= 1.0E-4F) {
         ...
         strafe *= f7;
         forward *= f7;
         float f8 = MathHelper.sin(mc.thePlayer.rotationYaw * 3.1415927F / 180.0F);
         float f9 = MathHelper.cos(mc.thePlayer.rotationYaw * 3.1415927F / 180.0F);
   ...
   public static Vec3 getPredictedPos(boolean isHitting, Entity targetEntity, float forward, float strafe) {
      ...
      float friction;
      if (isHitting) {
         f5 = (float)mc.thePlayer.getEntityAttribute(SharedMonsterAttributes.attackDamage).getAttributeValue();
         friction = 0.0F;
      ...
      if (mc.thePlayer.isJumping && mc.thePlayer.onGround && mc.thePlayer.jumpTicks == 0) {
         ...
         if (isSprinting) {
            f5 = mc.thePlayer.rotationYaw * 0.017453292F;
      ...
      f5 = 0.16277136F / (f4 * f4 * f4);
      if (mc.thePlayer.onGround) {
         friction = mc.thePlayer.getAIMoveSpeed() * f5;
      } else {
         friction = mc.thePlayer.jumpMovementFactor;
      }
      ...
      if (f8 >= 1.0E-4F) {
         ...
         strafe *= f8;
         forward *= f8;
         float f9 = MathHelper.sin(mc.thePlayer.rotationYaw * 3.1415927F / 180.0F);
         float f10 = MathHelper.cos(mc.thePlayer.rotationYaw * 3.1415927F / 180.0F);
