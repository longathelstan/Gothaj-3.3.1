...
public class Particle {
   ...
   public boolean draw() {
      ...
      if (!(this.time < (float)System.nanoTime() / 1000000.0F) && !(this.x < 0.0D) && !(this.y < 0.0D) && !(this.x > (double)sr.getScaledWidth()) && !(this.y > (double)sr.getScaledHeight())) {
   ...
   public void update() {
      this.x += Math.sin(Math.toRadians((double)this.yaw)) * this.speed;
      this.y += Math.cos(Math.toRadians((double)this.yaw)) * this.speed;
   }
