...
public class FontUtils {
   ...
   protected FontUtils.CharData[] charData;
   protected FontUtils.CharData[] boldChars;
   protected FontUtils.CharData[] italicChars;
   protected FontUtils.CharData[] boldItalicChars;
   protected Font font;
   ...
   public static double fontScaleOffset = 2.0D;
   ...
   public FontUtils(Font font, boolean antiAlias, boolean fractionalMetrics) {
      this.imgSize = 512.0F;
      this.charData = new FontUtils.CharData[256];
      this.boldChars = new FontUtils.CharData[256];
      this.italicChars = new FontUtils.CharData[256];
      this.boldItalicChars = new FontUtils.CharData[256];
      this.fontHeight = -1;
      this.charOffset = 0;
      this.colorCode = new int[32];
      this.colorcodeIdentifiers = "0123456789abcdefklmnor";
      this.COLOR_INVOKER = 167;
      this.font = font;
   ...
   public void drawHighlitedString(String text, String contain, double x, double y, int color, int hightLightColor) {
      if (text.toLowerCase().equals(contain.toLowerCase())) {
         ...
         if (text.toLowerCase().endsWith(contain.toLowerCase())) {
            double var10000 = spacing + (double)this.drawString(contain.toLowerCase(), x + spacing, y, hightLightColor, false, 9.0F);
         }
   ...
   public float drawStringWithShadow(String text, double x, double y, int color, int shadowColor) {
      this.drawString(text, x + 0.5D, y + 0.5D, shadowColor, false, 9.0F);
      return this.drawString(text, x, y, color, false, 9.0F);
   }
   ...
   public float drawString(String text, double x2, double y2, int color, boolean shadow, float kerning) {
      ...
      if (text == null) {
         ...
         boolean render = true;
         x2 *= 2.0D * fontScaleOffset;
         y2 = (y2 - 3.0D) * 2.0D * fontScaleOffset;
         GL11.glPushMatrix();
         GlStateManager.scale(0.5D / fontScaleOffset, 0.5D / fontScaleOffset, 0.5D / fontScaleOffset);
         GL11.glEnable(3042);
   ...
   public void drawChar(FontUtils.CharData[] chars, char c2, float x2, float y2) throws ArrayIndexOutOfBoundsException {
      try {
         this.drawQuad(x2, y2, (float)chars[c2].width, (float)chars[c2].height, (float)chars[c2].storedX, (float)chars[c2].storedY, (float)chars[c2].width, (float)chars[c2].height);
   ...
   public float getHeight(String text) {
      return (float)((double)((this.fontHeight - (int)(8.3D * fontScaleOffset)) / (int)(2.0D * fontScaleOffset)) - 2.0D * fontScaleOffset);
   }
   ...
   public double getWidth(String text) {
      ...
      return (double)width / (2.0D * fontScaleOffset);
   }
