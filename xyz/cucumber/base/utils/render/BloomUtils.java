...
public class BloomUtils {
   ...
   static {
      programId = Shaders.bloom.getProgramID();
   }
   ...
   public void post() {
      ...
      for(int i = 0; (double)i <= setting.radius.getValue(); ++i) {
         kernel.put(Kernel.calculateGaussianValue((float)i, (float)(setting.radius.getValue() / 2.0D)));
      }

      kernel.rewind();
      Shader var10000 = Shaders.bloom;
      Shader.uniform1f(programId, "u_radius", (float)setting.radius.getValue());
      var10000 = Shaders.bloom;
      ...
      Shaders.bloom.renderShader(0.0D, 0.0D, (double)this.sr.getScaledWidth(), (double)this.sr.getScaledHeight());
