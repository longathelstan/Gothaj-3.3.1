...
public class Fonts {
   ...
   public static FontUtils getFont(String font) {
      Iterator var2 = fonts.entrySet().iterator();

      while(var2.hasNext()) {
         Entry<String[], FontUtils> entry = (Entry)var2.next();
         Iterator var4 = Arrays.asList((String[])entry.getKey()).iterator();

         while(var4.hasNext()) {
            String s = (String)var4.next();
            boolean valid = s.toLowerCase().equals(font.toLowerCase());
            if (valid) {
               return (FontUtils)entry.getValue();
            }
         }
      }
