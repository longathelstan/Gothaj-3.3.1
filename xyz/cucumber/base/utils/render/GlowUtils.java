...
public class GlowUtils {
   ...
   public GlowUtils() {
      this.programId = Shaders.bloomESP.getProgramID();
   ...
   public void post(float saturation, float radius, float compression, long time, int mode, ColorSettings color) {
      ...
      kernel.rewind();
      Shader var10000 = Shaders.bloomESP;
      Shader.uniform1f(this.programId, "u_radius", radius);
      var10000 = Shaders.bloomESP;
      ...
      Shaders.bloomESP.renderShader(0.0D, 0.0D, (double)sr.getScaledWidth(), (double)sr.getScaledHeight());
      this.mc.getFramebuffer().bindFramebuffer(true);
      var10000 = Shaders.bloomESP;
