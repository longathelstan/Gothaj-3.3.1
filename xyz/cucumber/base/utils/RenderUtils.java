...
public class RenderUtils {
   ...
   public static void scissor(ScaledResolution scaledResolution, double x, double y, double width, double height) {
      ...
      GL11.glScissor((int)Math.round(x * (double)scaleFactor), (int)Math.round(((double)scaledResolution.getScaledHeight() - (y + height)) * (double)scaleFactor), (int)Math.round(width * (double)scaleFactor), (int)Math.round(height * (double)scaleFactor));
   }
   ...
   public static void drawArrowForClickGui(double x, double y, double size, int color, double angle) {
      ...
      color(color);
      GL11.glBegin(9);
      GL11.glVertex2d(x - size / 2.0D, y + size / 2.0D);
   ...
   public static void drawRoundedRectWithCorners(double x, double y, double x1, double y1, int color, double radius, boolean leftTop, boolean rightTop, boolean leftBot, boolean rightBot) {
      ...
      drawRect(x, y + (leftTop ? radius : 0.0D), x + radius, y1 - (leftBot ? radius : 0.0D), color);
      drawRect(x1 - radius, y + (rightTop ? radius : 0.0D), x1, y1 - (rightBot ? radius : 0.0D), color);
      if (leftTop) {
         drawCirclePart(x + radius, y + radius, radius, 270.0D, 360.0D, color);
      }

      if (rightTop) {
         drawCirclePart(x1 - radius, y + radius, radius, 0.0D, 90.0D, color);
      }

      if (leftBot) {
         drawCirclePart(x + radius, y1 - radius, radius, 180.0D, 270.0D, color);
      }

      if (rightBot) {
         drawCirclePart(x1 - radius, y1 - radius, radius, 90.0D, 180.0D, color);
      }
   ...
   public static void drawRoundedRect(double x, double y, double x1, double y1, int color, double radius) {
      ...
      for(i = 270.0D; i < 360.0D; ++i) {
         GL11.glVertex2d(xs + Math.sin(i * 3.141592653589793D / 180.0D) * radius, ys - Math.cos(i * 3.141592653589793D / 180.0D) * radius);
      }
   ...
   public static void drawMixedRoundedRect(double x, double y, double x1, double y1, int color1, int color2, double radius) {
      ...
      double xs = x + radius;
      double ys = y + radius;
      color(ColorUtils.getAlphaColor(ColorUtils.mix(color1, color2, Math.sin(Math.toRadians((double)(System.nanoTime() / 1000000L / 7L + 0L))) + 1.0D, 2.0D), (int)(alpha * 100.0F)));

      double i;
      for(i = 270.0D; i < 360.0D; i += 2.0D) {
         GL11.glVertex2d(xs + Math.sin(i * 3.141592653589793D / 180.0D) * radius, ys - Math.cos(i * 3.141592653589793D / 180.0D) * radius);
      }

      color(ColorUtils.getAlphaColor(ColorUtils.mix(color1, color2, Math.sin(Math.toRadians((double)(System.nanoTime() / 1000000L / 7L + 90L))) + 1.0D, 2.0D), (int)(alpha * 100.0F)));
      xs = x1 - radius;
      ys = y + radius;

      for(i = 0.0D; i < 90.0D; i += 2.0D) {
         GL11.glVertex2d(xs + Math.sin(i * 3.141592653589793D / 180.0D) * radius, ys - Math.cos(i * 3.141592653589793D / 180.0D) * radius);
      }

      xs = x1 - radius;
      ys = y1 - radius;
      color(ColorUtils.getAlphaColor(ColorUtils.mix(color1, color2, Math.sin(Math.toRadians((double)(System.nanoTime() / 1000000L / 7L + 180L))) + 1.0D, 2.0D), (int)(alpha * 100.0F)));

      for(i = 90.0D; i < 180.0D; i += 2.0D) {
         GL11.glVertex2d(xs + Math.sin(i * 3.141592653589793D / 180.0D) * radius, ys - Math.cos(i * 3.141592653589793D / 180.0D) * radius);
      }

      color(ColorUtils.getAlphaColor(ColorUtils.mix(color1, color2, Math.sin(Math.toRadians((double)(System.nanoTime() / 1000000L / 7L + 260L))) + 1.0D, 2.0D), (int)(alpha * 100.0F)));
      xs = x + radius;
      ys = y1 - radius;

      for(i = 180.0D; i < 270.0D; i += 2.0D) {
         GL11.glVertex2d(xs + Math.sin(i * 3.141592653589793D / 180.0D) * radius, ys - Math.cos(i * 3.141592653589793D / 180.0D) * radius);
      }
   ...
   public static void drawMixedRect(double x, double y, double x1, double y1, int color1, int color2) {
      ...
      GL11.glShadeModel(7425);
      GL11.glBegin(7);
      color(ColorUtils.getAlphaColor(ColorUtils.mix(color1, color2, Math.sin(Math.toRadians((double)(System.nanoTime() / 1000000L / 10L + 0L))) + 1.0D, 2.0D), 100));
      GL11.glVertex2d(x, y);
      color(ColorUtils.getAlphaColor(ColorUtils.mix(color1, color2, Math.sin(Math.toRadians((double)(System.nanoTime() / 1000000L / 10L + 90L))) + 1.0D, 2.0D), 100));
      GL11.glVertex2d(x1, y);
      color(ColorUtils.getAlphaColor(ColorUtils.mix(color1, color2, Math.sin(Math.toRadians((double)(System.nanoTime() / 1000000L / 10L + 180L))) + 1.0D, 2.0D), 100));
      GL11.glVertex2d(x1, y1);
      color(ColorUtils.getAlphaColor(ColorUtils.mix(color1, color2, Math.sin(Math.toRadians((double)(System.nanoTime() / 1000000L / 10L + 260L))) + 1.0D, 2.0D), 100));
      GL11.glVertex2d(x, y1);
      GL11.glEnd();
      stop2D();
   }
