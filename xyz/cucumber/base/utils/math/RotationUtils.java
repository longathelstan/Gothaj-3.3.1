...
public class RotationUtils {
   ...
   public static float fovToEntity(Entity ent) {
      ...
      double yaw = (double)((float)(MathHelper.atan2(z, x) * 180.0D / 3.141592653589793D - 90.0D));
      return (float)yaw;
   }
   ...
   public static float[] positionRotation(double posX, double posY, double posZ, float[] lastRots, float yawSpeed, float pitchSpeed, boolean random) {
      ...
      float calcYaw = (float)(MathHelper.atan2(z, x) * 180.0D / 3.141592653589793D - 90.0D);
      float calcPitch = (float)(-(MathHelper.atan2(y, (double)MathHelper.sqrt_double(x * x + z * z)) * 180.0D / 3.141592653589793D));
   ...
   public static boolean lookingAtBlock(BlockPos blockPos, float yaw, float pitch, EnumFacing enumFacing, boolean strict) {
      MovingObjectPosition movingObjectPosition = mc.thePlayer.rayTraceCustom((double)mc.playerController.getBlockReachDistance(), mc.timer.renderPartialTicks, yaw, pitch);
   ...
   public static float[] getRotationsToBlock(BlockPos blockPos, EnumFacing enumFacing) {
      double x = (double)blockPos.getX() + 0.5D - mc.thePlayer.posX + (double)enumFacing.getFrontOffsetX() / 2.0D;
      double z = (double)blockPos.getZ() + 0.5D - mc.thePlayer.posZ + (double)enumFacing.getFrontOffsetZ() / 2.0D;
      double y = (double)blockPos.getY() + 0.5D;
      double dist = mc.thePlayer.getDistance((double)blockPos.getX() + 0.5D + (double)enumFacing.getFrontOffsetX() / 2.0D, (double)blockPos.getY(), (double)blockPos.getZ() + 0.5D + (double)enumFacing.getFrontOffsetZ() / 2.0D);
      y += 0.5D;
   ...
   public static float getYawBasedPitch(BlockPos blockPos, EnumFacing facing, float currentYaw, float lastPitch, int maxPitch) {
      float increment = (float)(Math.random() / 20.0D) + 0.05F;

      for(float i = (float)maxPitch; i > 45.0F; i -= increment) {
         MovingObjectPosition ray = rayCast(1.0F, new float[]{currentYaw, i}, (double)mc.playerController.getBlockReachDistance(), 2.0D);
   ...
   public static MovingObjectPosition rayCast(float partialTicks, float[] rots, double range, double hitBoxExpand) {
      ...
      if (entity != null && mc.theWorld != null) {
         ...
         Vec3 vec6 = null;
         float f = 1.0F;
         List list = mc.theWorld.getEntitiesInAABBexcluding(entity, entity.getEntityBoundingBox().addCoord(vec4.xCoord * d0, vec4.yCoord * d0, vec4.zCoord * d0).expand(1.0D, 1.0D, 1.0D), Predicates.and(new Predicate[]{EntitySelectors.NOT_SPECTATING}));
         double d3 = d2;
   ...
   public static float[] getNormalAuraRotations(float yaw, float pitch, Entity target, double x, double y, double z, float yawSpeed, float pitchSpeed, boolean hitVec) {
      if (target != null) {
         pitchSpeed = (float)((double)pitchSpeed * 0.5D);
         if (yawSpeed < 0.0F) {
            yawSpeed *= -1.0F;
         }
   ...
   public static float[] getInstantTargetRotation(Entity ent, double x, double y, double z, boolean hitVec) {
      ...
      if (!hitVec) {
         ...
         double nearest = 15.0D;
         AxisAlignedBB boundingBox = ent.getEntityBoundingBox();

         for(double x1 = boundingBox.minX; x1 <= boundingBox.maxX; x1 += 0.07D) {
            for(double z1 = boundingBox.minZ; z1 <= boundingBox.maxZ; z1 += 0.07D) {
               for(double y1 = boundingBox.minY; y1 <= boundingBox.maxY; y1 += 0.07D) {
                  Vec3 pos = new Vec3(x1, y1, z1);
                  if (mc.thePlayer.canPosBeSeen(pos)) {
                     Vec3 eyes = mc.thePlayer.getPositionEyes(1.0F);
                     double dist = Math.sqrt(Math.pow(x1 - eyes.xCoord, 2.0D) + Math.pow(y1 - eyes.yCoord, 2.0D) + Math.pow(z1 - eyes.zCoord, 2.0D));
                     if (dist <= nearest) {
                        nearest = dist;
                        best = pos;
                     }
   ...
   public static Vec3 getVec3(BlockPos pos, EnumFacing face) {
      ...
      if (face != EnumFacing.UP && face != EnumFacing.DOWN) {
         y += (new Random()).nextDouble() / 2.0D - 0.25D;
      } else {
         x += (new Random()).nextDouble() / 2.0D - 0.25D;
         z += (new Random()).nextDouble() / 2.0D - 0.25D;
      }
