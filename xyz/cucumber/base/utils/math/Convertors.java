...
public class Convertors {
   public static float[] convert2D(float x, float y, float z, int scaleFactor) {
      ...
      return GLU.gluProject(x, y, z, modelMatrix, projectionMatrix, viewport, windowPosition) ? new float[]{windowPosition.get(0) / (float)scaleFactor, ((float)Display.getHeight() - windowPosition.get(1)) / (float)scaleFactor, windowPosition.get(2)} : null;
   }
}
