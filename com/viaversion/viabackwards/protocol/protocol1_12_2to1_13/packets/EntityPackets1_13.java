...
public class EntityPackets1_13 extends LegacyEntityRewriter<ClientboundPackets1_13, Protocol1_12_2To1_13> {
   ...
   protected void registerPackets() {
         ...
         public void register() {
            ...
            this.handler((wrapper) -> {
               if (ViaBackwards.getConfig().isFix1_13FacePlayer()) {
                  ...
                  playerStorage.setX(EntityPackets1_13.toSet(bitField, 0, playerStorage.getX(), (Double)wrapper.get(Type.DOUBLE, 0)));
                  playerStorage.setY(EntityPackets1_13.toSet(bitField, 1, playerStorage.getY(), (Double)wrapper.get(Type.DOUBLE, 1)));
                  playerStorage.setZ(EntityPackets1_13.toSet(bitField, 2, playerStorage.getZ(), (Double)wrapper.get(Type.DOUBLE, 2)));
               }
            });
         }
      });
      ((Protocol1_12_2To1_13)this.protocol).registerClientbound(ClientboundPackets1_13.SPAWN_ENTITY, new PacketHandlers() {
         public void register() {
            ...
            this.handler((wrapper) -> {
               Optional<EntityTypes1_13.ObjectType> optionalType = EntityTypes1_13.ObjectType.findById((Byte)wrapper.get(Type.BYTE, 0));
               if (optionalType.isPresent()) {
                  EntityTypes1_13.ObjectType type = (EntityTypes1_13.ObjectType)optionalType.get();
                  int data;
      ...
      ((Protocol1_12_2To1_13)this.protocol).registerClientbound(ClientboundPackets1_13.FACE_PLAYER, (ClientboundPacketType)null, (wrapper) -> {
         wrapper.cancel();
         if (ViaBackwards.getConfig().isFix1_13FacePlayer()) {
   ...
   protected void registerRewrites() {
      ...
      this.mapEntityTypeWithData(EntityTypes1_13.EntityType.PHANTOM, EntityTypes1_13.EntityType.PARROT).plainName().spawnMetadata((storage) -> {
      ...
      this.filter().handler((event, meta) -> {
         ...
         JsonElement element;
         if (typeId == 4) {
            element = (JsonElement)meta.value();
      ...
      this.filter().type(EntityTypes1_13.EntityType.WOLF).index(17).handler((event, meta) -> {
