...
public class InventoryPackets {
   public static void register(final Protocol1_7_6_10To1_8 protocol) {
      protocol.registerClientbound(ClientboundPackets1_8.OPEN_WINDOW, (wrapper) -> {
         ...
         short windowTypeId = InventoryTracker.getInventoryType((String)wrapper.read(Type.STRING));
         windowTracker.getWindowTypeMap().put(windowId, windowTypeId);
         wrapper.write(Type.UNSIGNED_BYTE, windowTypeId);
         JsonElement titleComponent = (JsonElement)wrapper.read(Type.COMPONENT);
      ...
      protocol.registerClientbound(ClientboundPackets1_8.SET_SLOT, new PacketHandlers() {
         public void register() {
            ...
            this.handler((wrapper) -> {
               short windowType = ((InventoryTracker)wrapper.user().get(InventoryTracker.class)).get((Short)wrapper.get(Type.UNSIGNED_BYTE, 0));
      ...
      protocol.registerClientbound(ClientboundPackets1_8.WINDOW_ITEMS, new PacketHandlers() {
         public void register() {
            ...
            this.handler((wrapper) -> {
               short windowType = ((InventoryTracker)wrapper.user().get(InventoryTracker.class)).get((Short)wrapper.get(Type.UNSIGNED_BYTE, 0));
               Item[] items = (Item[])wrapper.read(Type.ITEM1_8_SHORT_ARRAY);
               if (windowType == 4) {
                  Item[] old = items;
            ...
            this.handler((wrapper) -> {
               ...
               if (windowId == 0) {
                  ...
                  PlayerSessionStorage playerSession = (PlayerSessionStorage)wrapper.user().get(PlayerSessionStorage.class);
                  Item[] items = (Item[])wrapper.get(Types1_7_6_10.COMPRESSED_NBT_ITEM_ARRAY, 0);
      ...
      protocol.registerClientbound(ClientboundPackets1_8.WINDOW_PROPERTY, new PacketHandlers() {
         public void register() {
            ...
            this.handler((wrapper) -> {
               ...
               short progressBarId = (Short)wrapper.get(Type.SHORT, 0);
               short progress = (Short)wrapper.get(Type.SHORT, 1);
               if (windowType == 2) {
                  FurnaceData furnace = (FurnaceData)windowTracker.getFurnaceData().computeIfAbsent(windowId, (x) -> {
                  ...
                  if (progressBarId != 0 && progressBarId != 1) {
                     if (progressBarId == 2 || progressBarId == 3) {
                        ...
                        wrapper.set(Type.SHORT, 0, Short.valueOf((short)0));
                     ...
                     wrapper.set(Type.SHORT, 0, Short.valueOf((short)1));
                     wrapper.set(Type.SHORT, 1, progress);
                  }
