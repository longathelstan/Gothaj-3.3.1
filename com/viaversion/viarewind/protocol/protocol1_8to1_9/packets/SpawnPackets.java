               ...
               if (typeId != 3 && typeId != 91 && typeId != 92 && typeId != 93) {
                  if (type == null) {
                     ...
                     if (type.is(EntityTypes1_10.EntityType.BOAT)) {
                        ...
                        shulkerBulletReplacement.updateReplacementPosition((double)x / 32.0D, (double)y / 32.0D, (double)z / 32.0D);
         ...
         public void register() {
            ...
            this.handler((packetWrapper) -> {
               ...
               if (typeId == 69) {
                  ...
                  shulkerReplacement.updateReplacementPosition((double)x / 32.0D, (double)y / 32.0D, (double)z / 32.0D);
      ...
      protocol.registerClientbound(ClientboundPackets1_9.SPAWN_PLAYER, new PacketHandlers() {
         public void register() {
            ...
            this.handler((wrapper) -> {
               int entityId = (Integer)wrapper.get(Type.VAR_INT, 0);
               List<Metadata> metadataList = (List)wrapper.get(Types1_8.METADATA_LIST, 0);
               protocol.getMetadataRewriter().transform((EntityTracker)wrapper.user().get(EntityTracker.class), entityId, metadataList, EntityTypes1_10.EntityType.PLAYER);
            });
            this.handler((packetWrapper) -> {
               int entityId = (Integer)packetWrapper.get(Type.VAR_INT, 0);
               EntityTracker tracker = (EntityTracker)packetWrapper.user().get(EntityTracker.class);
