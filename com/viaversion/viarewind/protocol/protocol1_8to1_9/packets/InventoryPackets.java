   ...
   public static void register(final Protocol1_8To1_9 protocol) {
      ...
      protocol.registerClientbound(ClientboundPackets1_9.WINDOW_ITEMS, new PacketHandlers() {
         public void register() {
            ...
            this.handler((packetWrapper) -> {
               short windowId = (Short)packetWrapper.get(Type.UNSIGNED_BYTE, 0);
               Item[] items = (Item[])packetWrapper.read(Type.ITEM1_8_SHORT_ARRAY);
                  ...
                  if (type != null) {
                     if (type.equalsIgnoreCase("minecraft:brewing_stand")) {
                        if (slot > 4) {
                           ...
                           packetWrapper.cancel();
                           Windows.updateBrewingStand(packetWrapper.user(), (Item)packetWrapper.get(Type.ITEM1_8, 0), (short)windowId);
                        } else {
                           ((Windows)packetWrapper.user().get(Windows.class)).getBrewingItems((short)windowId)[slot] = (Item)packetWrapper.get(Type.ITEM1_8, 0);
                        }
                     }
      ...
      protocol.registerServerbound(ServerboundPackets1_8.CLICK_WINDOW, new PacketHandlers() {
         public void register() {
            ...
            this.map(Type.ITEM1_8);
            this.handler((packetWrapper) -> {
               packetWrapper.set(Type.ITEM1_8, 0, protocol.getItemRewriter().handleItemToServer((Item)packetWrapper.get(Type.ITEM1_8, 0)));
            });
            this.handler((packetWrapper) -> {
               short windowId = (Short)packetWrapper.get(Type.UNSIGNED_BYTE, 0);
