...
public class TitleCooldownVisualization implements CooldownVisualization {
   ...
   private void sendTitle(String titleText, String subTitleText, int fadeIn, int stay, int fadeOut) throws Exception {
      this.sendTitlePacket(0, (packet) -> {
         packet.write(Type.COMPONENT, new JsonPrimitive(titleText));
      });
      this.sendTitlePacket(1, (packet) -> {
         packet.write(Type.COMPONENT, new JsonPrimitive(subTitleText));
      });
   ...
   private void sendTitlePacket(int action, Consumer<PacketWrapper> writer) throws Exception {
      PacketWrapper titlePacket = PacketWrapper.create(ClientboundPackets1_8.TITLE, (UserConnection)this.user);
