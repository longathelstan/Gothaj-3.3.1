...
public class Windows extends StoredObject {
   ...
   public static void updateBrewingStand(UserConnection user, Item blazePowder, short windowId) {
      if (blazePowder == null || blazePowder.identifier() == 377) {
         ...
         PacketWrapper openWindow = PacketWrapper.create(ClientboundPackets1_8.OPEN_WINDOW, (UserConnection)user);
         openWindow.write(Type.UNSIGNED_BYTE, windowId);
         openWindow.write(Type.STRING, "minecraft:brewing_stand");
         ATextComponent title = (new StringComponent()).append(new TranslationComponent("container.brewing", new Object[0])).append((ATextComponent)(new StringComponent(": " + TextFormatting.DARK_GRAY))).append((ATextComponent)(new StringComponent(amount + " " + TextFormatting.DARK_RED))).append((ATextComponent)(new TranslationComponent("item.blazePowder.name", new Object[]{TextFormatting.DARK_RED})));
         ...
         for(int i = 0; i < items.length; ++i) {
            PacketWrapper setSlot = PacketWrapper.create(ClientboundPackets1_8.SET_SLOT, (UserConnection)user);
