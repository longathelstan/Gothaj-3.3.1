...
public class MetadataRewriter {
   ...
   public void transform(EntityTracker tracker, int entityId, List<Metadata> list) {
      this.transform(tracker, entityId, list, (EntityTypes1_10.EntityType)tracker.getClientEntityTypes().get(entityId));
   }
   ...
   public void transform(EntityTracker tracker, int entityId, List<Metadata> list, EntityTypes1_10.EntityType entityType) {
      ...
      Metadata entry;
      while(var5.hasNext()) {
         entry = (Metadata)var5.next();
         if (entry.id() == MetaIndex.ENTITY_STATUS.getIndex()) {
            tracker.getStatusInformation().put(entityId, (Byte)entry.getValue());
         }
      }

      var5 = (new ArrayList(list)).iterator();
         ...
         try {
            ...
            if (metaIndex.getOldType() != MetaType1_8.NonExistent && metaIndex.getNewType() != null) {
               ...
               if (metaIndex == MetaIndex.PLAYER_HAND) {
                  byte status = (Byte)tracker.getStatusInformation().getOrDefault(entityId, (byte)0);
                  if (((Byte)entry.value() & 1) != 0) {
