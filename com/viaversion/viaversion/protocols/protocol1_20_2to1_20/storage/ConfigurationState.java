...
public class ConfigurationState implements StorableObject {
   ...
   private ConfigurationState.BridgePhase bridgePhase;
   ...
   public void onRemove() {
      Iterator var1 = this.packetQueue.iterator();

      while(var1.hasNext()) {
         ConfigurationState.QueuedPacket packet = (ConfigurationState.QueuedPacket)var1.next();
         packet.buf().release();
      }
   ...
   public void sendQueuedPackets(UserConnection connection) throws Exception {
      ...
      ConfigurationState.QueuedPacket[] queuedPackets = (ConfigurationState.QueuedPacket[])this.packetQueue.toArray(EMPTY_PACKET_ARRAY);
      this.packetQueue.clear();
      ConfigurationState.QueuedPacket[] var5 = queuedPackets;
      int var6 = queuedPackets.length;

      for(int var7 = 0; var7 < var6; ++var7) {
         ConfigurationState.QueuedPacket packet = var5[var7];
   ...
   public PacketWrapper clientInformationPacket(UserConnection connection) {
      if (this.clientInformation == null) {
         ...
         PacketWrapper settingsPacket = PacketWrapper.create(ServerboundPackets1_19_4.CLIENT_SETTINGS, (UserConnection)connection);
   ...
   public static final class QueuedPacket {
      ...
      // $FF: synthetic method
      QueuedPacket(ByteBuf x0, boolean x1, PacketType x2, int x3, boolean x4, Object x5) {
         this(x0, x1, x2, x3, x4);
      }
   }
