...
public class EntityTracker1_9 extends EntityTrackerBase {
   ...
   public void setSecondHand(int entityID, Item item) {
      PacketWrapper wrapper = PacketWrapper.create(ClientboundPackets1_9.ENTITY_EQUIPMENT, (ByteBuf)null, this.user());
   ...
   public void handleMetadata(int entityId, List<Metadata> metadataList) {
      ...
      if (type != null) {
         ...
         while(true) {
               ...
               do {
                  ...
                  if (type == EntityTypes1_10.EntityType.ARMOR_STAND && Via.getConfig().isHologramPatch() && metadata.id() == 0 && this.getMetaByIndex(metadataList, 10) != null) {
                     ...
                     if ((data & 32) == 32 && ((Byte)meta.getValue() & 1) == 1 && (displayName = this.getMetaByIndex(metadataList, 2)) != null && !((String)displayName.getValue()).isEmpty() && (displayNameVisible = this.getMetaByIndex(metadataList, 3)) != null && (Boolean)displayNameVisible.getValue() && !this.knownHolograms.contains(entityId)) {
                        ...
                        try {
                           PacketWrapper wrapper = PacketWrapper.create(ClientboundPackets1_9.ENTITY_POSITION, (ByteBuf)null, this.user());
                           wrapper.write(Type.VAR_INT, entityId);
            ...
            } while(type != EntityTypes1_10.EntityType.ENDER_DRAGON && type != EntityTypes1_10.EntityType.WITHER);

            BossBar bar;
            if (metadata.id() == 2) {
               bar = (BossBar)this.bossBarMap.get(entityId);
               String title = (String)metadata.getValue();
               title = title.isEmpty() ? (type == EntityTypes1_10.EntityType.ENDER_DRAGON ? "{\"translate\":\"entity.EnderDragon.name\"}" : "{\"translate\":\"entity.WitherBoss.name\"}") : title;
               if (bar == null) {
               ...
               if (bar == null) {
                  String title = type == EntityTypes1_10.EntityType.ENDER_DRAGON ? "{\"translate\":\"entity.EnderDragon.name\"}" : "{\"translate\":\"entity.WitherBoss.name\"}";
   ...
   public void sendTeamPacket(boolean add, boolean now) {
      PacketWrapper wrapper = PacketWrapper.create(ClientboundPackets1_9.TEAMS, (ByteBuf)null, this.user());
   ...
   public void addMetadataToBuffer(int entityID, List<Metadata> metadataList) {
      List<Metadata> metadata = (List)this.metadataBuffer.get(entityID);
      if (metadata != null) {
   ...
   public void sendMetadataBuffer(int entityId) {
      List<Metadata> metadataList = (List)this.metadataBuffer.get(entityId);
      if (metadataList != null) {
         PacketWrapper wrapper = PacketWrapper.create(ClientboundPackets1_9.ENTITY_METADATA, (ByteBuf)null, this.user());
         wrapper.write(Type.VAR_INT, entityId);
