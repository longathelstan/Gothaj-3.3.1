...
public class SNbtDeserializer_v1_12 implements ISNbtDeserializer<CompoundTag> {
   ...
   protected Tag readArray(StringReader_v1_12 reader) throws SNbtDeserializeException {
      ...
      if (!reader.canRead()) {
         ...
         int i;
         if (c == 'B') {
            tags = this.readPrimitiveList(reader, ByteTag.class, ByteArrayTag.class);
            byte[] array = new byte[tags.size()];

            for(i = 0; i < tags.size(); ++i) {
               array[i] = (Byte)tags.get(i).value();
            }

            return new ByteArrayTag(array);
         } else if (c == 'L') {
            tags = this.readPrimitiveList(reader, LongTag.class, LongArrayTag.class);
            long[] array = new long[tags.size()];

            for(i = 0; i < tags.size(); ++i) {
               array[i] = (Long)tags.get(i).value();
            }
            ...
            tags = this.readPrimitiveList(reader, IntTag.class, IntArrayTag.class);
            int[] array = new int[tags.size()];

            for(i = 0; i < tags.size(); ++i) {
               array[i] = (Integer)tags.get(i).value();
            }
