...
public class ArmorListener extends ViaBukkitListener {
   ...
   public void sendArmorUpdate(Player player) {
      if (this.isOnPipe(player)) {
         int armor = 0;
         ItemStack[] var3 = player.getInventory().getArmorContents();
         ...
         PacketWrapper wrapper = PacketWrapper.create(ClientboundPackets1_9.ENTITY_PROPERTIES, (ByteBuf)null, this.getUserConnection(player));

         try {
            wrapper.write(Type.VAR_INT, player.getEntityId());
            wrapper.write(Type.INT, 1);
            wrapper.write(Type.STRING, "generic.armor");
            wrapper.write(Type.DOUBLE, 0.0D);
