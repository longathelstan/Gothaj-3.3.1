...
public final class ProtocolSupportCompat {
   public static void registerPSConnectListener(ViaVersionPlugin plugin) {
      ...
      try {
         Class<? extends Event> connectionOpenEvent = Class.forName("protocolsupport.api.events.ConnectionOpenEvent");
         Bukkit.getPluginManager().registerEvent(connectionOpenEvent, new Listener() {
            ...
            try {
               ...
               Via.getPlatform().getLogger().log(Level.WARNING, "Error when handling ProtocolSupport event", var4);
            }

         }, plugin);
      } catch (ClassNotFoundException var2) {
         Via.getPlatform().getLogger().log(Level.WARNING, "Unable to register ProtocolSupport listener", var2);
      }
   ...
   static ProtocolSupportCompat.HandshakeProtocolType handshakeVersionMethod() {
      ...
      try {
         ...
         try {
            if (clazz.getMethod("b").getReturnType() == Integer.TYPE) {
               return ProtocolSupportCompat.HandshakeProtocolType.OBFUSCATED_B;
            } else if (clazz.getMethod("c").getReturnType() == Integer.TYPE) {
